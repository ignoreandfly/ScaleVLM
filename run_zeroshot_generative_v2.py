...
import pandas as pd
import torch
from transformers import AutoProcessor, AutoModelForCausalLM, BlipProcessor, BlipForQuestionAnswering, ViltProcessor, ViltForQuestionAnswering
...
class GenerativeVLMBenchmark:
    def __init__(self, model_name="microsoft/git-base-vqav2", device=None):
        """
        Initialize VLM benchmark with fast VQA models:
        - "microsoft/git-base-vqav2" - GIT VQA model (~300M parameters)
        - "dandelin/vilt-b32-finetuned-vqa" - ViLT VQA model (~200M parameters)
        - "Salesforce/blip-vqa-base" - BLIP VQA model (kept for comparison)
        """
    ...
    def answer_vqa_question(self, image_path, question):
        ...
        try:
            ...
            if "blip" in self.model_name.lower():
            ...
            elif "vilt" in self.model_name.lower():
                # ViLT specific processing
                inputs = self.processor(image, question, return_tensors="pt").to(self.device)
                ...
                with torch.no_grad():
                    ...
                    answer = self.model.config.id2label[predicted_class_idx]
            
            elif "git" in self.model_name.lower():
                # GIT VQA specific processing
                ...
                with torch.no_grad():
                    ...
                    # Clean up the answer - remove the question part
...
def main():
    """Main benchmarking function - testing fast VQA models"""
    
    # Configuration - Updated to use fast models
    MODEL_NAME = "dandelin/vilt-b32-finetuned-vqa"  # Start with fastest model
    ...
    print(f"🚀 Starting VLM Benchmark for {MODEL_NAME}...")
    ...
    try:
        ...
        print(f"✅ Completed benchmarking {MODEL_NAME}")
        
        if results_df is not None:
            accuracy = results_df['correct'].mean()
            total = len(results_df)
            print(f"\nFinal Result: {accuracy:.3f} ({accuracy*100:.1f}%) accuracy on {total} questions")
...
def benchmark_fast_models():
    """Benchmark the two fastest VQA models for quick results"""
    
    # Two fastest VQA models
    models = [
        "dandelin/vilt-b32-finetuned-vqa",    # ~200M parameters - Fastest
        "microsoft/git-base-vqav2"            # ~300M parameters - Very Fast
    ]
    ...
    for model_name in models:
        ...
        try:
            benchmark = GenerativeVLMBenchmark(model_name=model_name)
            results[model_name] = benchmark.evaluate_vqa_dataset(VQA_FILE, OUTPUT_DIR)
            
            # Print quick summary
            if results[model_name] is not None:
                accuracy = results[model_name]['correct'].mean()
                total = len(results[model_name])
                print(f"✅ {model_name}: {accuracy:.3f} ({accuracy*100:.1f}%) accuracy on {total} questions")
    ...
    # Compare results
    print(f"\n{'='*80}")
    print("FINAL COMPARISON - FAST VQA MODELS")
    print('='*80)
    ...
    for model_name, result_df in results.items():
        if result_df is not None:
            ...
            print(f"{model_short:30}: {accuracy:.3f} ({accuracy*100:.1f}%) on {total} questions")
    
    return results

if __name__ == "__main__":
    # Run the fast benchmark comparison
