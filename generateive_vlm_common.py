...
import pandas as pd
import torch
from transformers import AutoProcessor, AutoModelForCausalLM, BlipProcessor, BlipForQuestionAnswering, Qwen2VLForConditionalGeneration
...
class GenerativeVLMBenchmark:
    def __init__(self, model_name="Salesforce/blip-vqa-base", device=None):
        ...
        - "Qwen/Qwen2-VL-2B-Instruct" - Qwen2-VL model
        - "OpenGVLab/InternVL2-2B" - InternVL2 model
        """
    ...
    def answer_vqa_question(self, image_path, question):
        ...
        try:
            ...
            if "blip" in self.model_name.lower():
                ...
                image_inputs, video_inputs = self.processor.process(text=text, images=[image], videos=None, padding=True, return_tensors="pt")
                
                inputs = image_inputs.to(self.device)
            ...
            elif "internvl2" in self.model_name.lower():
...
def main():
    """Main benchmarking function - now benchmarks all three models"""
    ...
    print("🚀 Starting VLM Benchmark for all three models...")
    ...
    for model_name, df in results.items():
        if df is not None:
            ...
            total = len(df)
            print(f"{model_name}: {accuracy:.3f} ({accuracy*100:.1f}%) - {total} questions")
        else:
...
def benchmark_multiple_models():
    """Benchmark multiple VLM models for comparison - updated to include new models"""
