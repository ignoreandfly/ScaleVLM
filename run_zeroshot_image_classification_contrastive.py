...
class ExtendedCLIPObjectSizeBenchmark:
    def __init__(self, model_name="openai/clip-vit-base-patch32", model_type="hf_clip", device=None):
        """
        Initialize CLIP benchmark supporting HF CLIP, SigLIP, and OpenCLIP
        
        Args:
            model_name: Model identifier
                       HF CLIP: "openai/clip-vit-base-patch32", "openai/clip-vit-base-patch16", etc.
                       SigLIP: "google/siglip-base-patch16-224", "google/siglip-large-patch16-384", etc.
                       OpenCLIP: "ViT-B-32", "ViT-L-14", etc.
            model_type: "hf_clip", "siglip", or "openclip"
        ...
        print(f"Loading {model_type.upper()} model: {model_name} on {self.device}")
        
        # Load model and processor based on type
        if model_type == "hf_clip":
            ...
            # For OpenCLIP, model_name should be like "ViT-B-32" and we need pretrained dataset
            pretrained = "openai"  # Default, can be made configurable
            self.model, _, self.preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained)
            self.model = self.model.to(self.device)
            self.tokenizer = open_clip.get_tokenizer(model_name)
            self.processor = None  # OpenCLIP uses different interface
        else:
        ...
        try:
            ...
            if self.model_type == "openclip":
                ...
                with torch.no_grad():
                    ...
                    # For SigLIP, use sigmoid instead of softmax
    ...
    def benchmark_dataset(self, csv_path, output_dir="extended_clip_benchmark_results"):
        """
    ...
    def _analyze_results(self, results_df, output_dir, timestamp):
        ...
        print("\n" + "="*80)
        print("EXTENDED CLIP BENCHMARK ANALYSIS")
        print("="*80)
    ...
    def _create_visualizations(self, results_df, output_dir, timestamp):
        ...
        fig.suptitle(f'{self.model_type.upper()} Object Size Analysis - {self.model_name}', fontsize=16, fontweight='bold')
...
def main():
    ...
    OUTPUT_DIR = "/data/azfarm/siddhant/ICCV/extended_clip_benchmark_results"
    
    results_dict = {}
    
    for config in configs:
        print(f"\n{'='*80}")
        print(f"TESTING: {config['model_type'].upper()} - {config['model_name']}")
        print('='*80)
        
        try:
            # Initialize benchmark
            benchmark = ExtendedCLIPObjectSizeBenchmark(
            ...
            results_dict[f"{config['model_type']}_{config['model_name']}"] = results_df
...
def compare_all_models(csv_path, configs=None):
    """Compare multiple models across different architectures"""
    if configs is None:
        configs = [
            # HF CLIP models
            {"model_name": "openai/clip-vit-base-patch32", "model_type": "hf_clip"},
            {"model_name": "openai/clip-vit-base-patch16", "model_type": "hf_clip"},
            {"model_name": "openai/clip-vit-large-patch14", "model_type": "hf_clip"},
            
            # SigLIP models
            {"model_name": "google/siglip-base-patch16-224", "model_type": "siglip"},
            {"model_name": "google/siglip-large-patch16-384", "model_type": "siglip"},
            
            # OpenCLIP models
            {"model_name": "ViT-B-32", "model_type": "openclip"},
            {"model_name": "ViT-L-14", "model_type": "openclip"},
        ]
    
    results = {}
    
    for config in configs:
        model_id = f"{config['model_type']}_{config['model_name']}"
        print(f"\n{'='*60}")
        ...
        try:
            ...
            results[model_id] = benchmark.benchmark_dataset(csv_path)
        except Exception as e:
            print(f"Error with {model_id}: {e}")
    
    return results

def analyze_size_performance_extended(csv_path, model_configs, size_categories=['tiny', 'small', 'medium', 'large', 'huge']):
    """Detailed analysis of performance across size categories for multiple model types"""
    
    for config in model_configs:
        model_id = f"{config['model_type']}_{config['model_name']}"
        print(f"\n{'='*80}")
        print(f"DETAILED SIZE ANALYSIS: {model_id}")
        print("-" * 80)
